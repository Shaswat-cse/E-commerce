...
var prodLocalStorage = JSON.parse(localStorage.getItem("productsLS"));
var index = JSON.parse(localStorage.getItem("productIndex"));
var ss = JSON.parse(sessionStorage.getItem("detailsID"));
ss = parseInt(ss);

function addCart(id) {
  sessionStorage.setItem("productsID", JSON.stringify(id));

  var sp = JSON.parse(sessionStorage.getItem("productsID"));
  sp = parseInt(sp);

  var obj = {
    prodId: homePageDetails[sp].id,
    previewImageSrc: homePageDetails[sp].preview,
    prodName: homePageDetails[sp].name,
    prodPrice: homePageDetails[sp].price,
  };
  var mainind = {
    indi: id + 1,
  };
  if (prodLocalStorage == null) {
    ...
    localStorage.setItem("productsLS", JSON.stringify(prod));
  } else {
    prodLocalStorage.push(obj);

    localStorage.setItem("productsLS", JSON.stringify(prodLocalStorage));
  }

  if (index == null) {
    var ind = [];
    ind.push(mainind);
    localStorage.setItem("productIndex", JSON.stringify(ind));
  } else {
    index.push(mainind);
...
for (var i = 0; i < index.length; i++) {
  newarr[i] = index[i].indi;
}

var uniqueChars = [...new Set(newarr)];
for (var i = 0; i < uniqueChars.length; i++) {
  console.log(uniqueChars[i]);
}
var checkoutcard = document.getElementById("card-list");
var prodCount = document.getElementById("item-count");
prodCount.innerHTML = uniqueChars.length;

for (var i = 0; i < uniqueChars.length; i++) {
  var count = 0;
  var k = 0;
  for (var j = 0; j < prodLocalStorage.length; j++) {
    if (uniqueChars[i] == prodLocalStorage[j].prodId) {
      count++;
      k = j;
    } else {
      continue;
...
src="${prodLocalStorage[k].previewImageSrc}">
</div>
<div>
<h4>${prodLocalStorage[k].prodName}</h4>
<p id = "noOfProd" >x${count}</p>
<p><span>Amount: Rs </span><span>${prodLocalStorage[k].prodPrice}</span></p>
</div>
</div>`;
  }
  //    var noOfProd  = document.getElementById("noOfProd");
  //    noOfProd.innerHTML = `x${count}`;
}
function scpage() {
  location.assign("successpage.html");
}

var TotalAmount = 0;
if (prodLocalStorage.length == 0) {
  var totalAmount = document.getElementById("total-amount");
  totalAmount.innerHTML = 0;
} else {
  for (var i = 0; i < prodLocalStorage.length; i++) {
    TotalAmount += prodLocalStorage[i].prodPrice;
  }
  var totalAmount = document.getElementById("total-amount");
...
cartCount.innerHTML = prodLocalStorage.length;
function gocart() {

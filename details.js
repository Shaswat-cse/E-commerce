...
var detailspage = document.getElementById("details_page");
...
function detailsfunction(id) {
  ...
  console.log(sp);
  location.assign("http://127.0.0.1:5500/details.html");
}
var sp = JSON.parse(sessionStorage.getItem("detailsID"));
detailspage.innerHTML = `
<div id="detailsPart1">

            <img
                src="${homePageDetails[parseInt(sp)].preview}">
        </div>
        <div id="detailsPart2">
            <h1 class="prod_title">
               ${homePageDetails[parseInt(sp)].name}</h1>
            <h1 class="prod_brand">${homePageDetails[parseInt(sp)].brand}</h1>
            <h4 class="section-heading">
                Price Rs.
                <p id="prod_price">${homePageDetails[parseInt(sp)].price}</p>
            </h4>
            <p id="description">${homePageDetails[parseInt(sp)].description}</p>
            <p class="prod_prev">Product Preview</p>
            <div id="product-images">
            </div>
            <button id="btn-add-to-cart" onclick = addCart(${parseInt(
              sp,
            )}) >Add to Cart</button>

        </div>
`;
var detailsPageImgs = document.getElementById("product-images");
for (var i = 0; i < homePageDetails[parseInt(sp)].photos.length; i++) {
  if (i == 0) {
    detailsPageImgs.innerHTML += `<img onclick=addBorder(${i}) id = "card0" class="active-card" src="${
      homePageDetails[parseInt(sp)].photos[i]
    }">`;
  } else {
    detailsPageImgs.innerHTML += `<img onclick=addBorder(${i}) id="card${i}"  src="${
      homePageDetails[parseInt(sp)].photos[i]
    }">`;
  }
}
var detailspagePreview = document.getElementById("detailsPart1");
function addBorder(id) {
  $(".active-card").removeClass("active-card");
  $("#card" + id).addClass("active-card");
  console.log(`details clicked${id}`);
  detailspagePreview.innerHTML = `
  <img
      src="${homePageDetails[parseInt(sp)].photos[id]}">
`;
}
